...
const UserModal = ({ setModal }) => {
  ...
  const userModal = [
    {title: "Profile", icon: <LiaUserSolid/>         , path: `/profile/${currentUser?.uid}`}, // Profile icon
    {title: "Library", icon: <MdOutlineLocalLibrary/>, path: `/library`                    }, // Library icon
  ...
  return (
    <section 
      className='absolute w-[18rem] p-6 bg-white right-0 top-[100%]
        ...
        <Link to={"/write"} className="flex md:hidden items-center gap-1 text-grey-500">
        ...
        <div className='flex flex-col gap-4 border-b border-gray-300 pb-5'>
          {userModal.map((link, i) => ( // four userModal icons
            <Link 
