...
const Recommended = ({post: singlePost}) => {
  ...
  useEffect(() => {
    ...
    data && data.forEach((post) => {
        // exclude the current post from recommended
        if (post.id === singlePost.id) {
            return;
        }
        const postTag = post.tags;
        const commonTags = postTag.filter(
            (tag) => singlePost?.tags.includes(tag)
        );
            ...
            <h2 className='text-xl font-bold'>Recommended from Medium</h2>
            {
                ...
                (
                    <div className='grid grid-cols-card gap-[2rem] my-[3rem]'>
                        {commonTags.map((post) => <Post post={post} key={post.id}/>)} {/* Recommended Posts */}
                    </div>
                )
            }
        </div>
...
const Post = ({post}) => {
    ...
    const {username, userImg} = data && data.find((user) => user?.id===userId);
